...
export function ProfileForm({ profile }: { profile: DealerProfile }) {
  ...
  return (
    <form onSubmit={save} className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
      {/* Editable fields */}
      <div className="rounded bg-[#161616] border border-white/8 p-5 space-y-4">
        <div>
      ...
      <div className="space-y-3">
        <div className="rounded bg-bento-dark border border-[#D4AF37]/25 p-5 grain">
          <div className="h-14 w-14 rounded-sm bg-gradient-to-br from-[#F0CE5C] via-[#D4AF37] to-[#8C7220] flex items-center justify-center text-[#1A1208] font-black">
            {form.businessName.charAt(0)}
          </div>
          <div className="mt-3 flex items-center gap-1.5">
            <h3 className="font-semibold text-sm">{form.businessName}</h3>
            {profile.isVerified && <BadgeCheck className="h-4 w-4 text-[#F0CE5C]" />}
          </div>
        ...
        <div className="rounded bg-[#161616] border border-white/8 p-4 text-[11px] text-muted">
          Your public storefront:{" "}
          <a
            href={`/dealers/${profile.slug}`}
            target="_blank"
            rel="noreferrer"
            className="text-[#F0CE5C] hover:underline break-all"
          >
