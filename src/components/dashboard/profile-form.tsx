...
export function ProfileForm({ profile }: { profile: DealerProfile }) {
  ...
  return (
    <form onSubmit={save} className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
      {/* Editable fields */}
      <div className="rounded-2xl bg-white border border-[#E7E4DA] shadow-card p-5 space-y-4">
        <div>
        ...
        <div>
          ...
          <textarea
            className="w-full rounded-xl bg-white border border-[#E7E4DA] px-3 py-2 text-sm text-[#141414] placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-[#141414]/20 min-h-[90px]"
      ...
      <div className="space-y-3">
        <div className="rounded-2xl bg-bento-dark border border-[#F0941F]/25 shadow-card p-5 grain">
          <div className="h-14 w-14 rounded-xl bg-[#141414] flex items-center justify-center text-white font-black">
            {form.businessName.charAt(0)}
          </div>
          <div className="mt-3 flex items-center gap-1.5">
            <h3 className="font-semibold text-sm">{form.businessName}</h3>
            {profile.isVerified && <BadgeCheck className="h-4 w-4 text-[#F0941F]" />}
          </div>
        ...
        <div className="rounded-2xl bg-white border border-[#E7E4DA] shadow-card p-4 text-[11px] text-muted">
          Your public storefront:{" "}
          <a
            href={`/dealers/${profile.slug}`}
            target="_blank"
            rel="noreferrer"
            className="text-[#C97612] hover:underline break-all"
          >
