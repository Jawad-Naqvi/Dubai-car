  ...
  return (
    <div>
      {items.map((item, i) => {
        const isOpen = open === i;
        return (
          <div key={item.q} className="border-b border-[#E5E5EA] last:border-0">
            <button
              onClick={() => setOpen(isOpen ? null : i)}
              className="w-full flex items-center justify-between gap-4 py-4 text-start group"
              aria-expanded={isOpen}
            >
              <span className="text-sm lg:text-base font-semibold text-[#141414] group-hover:text-[#6B21A8] transition-colors">
                {item.q}
              </span>
