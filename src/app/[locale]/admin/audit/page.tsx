    ...
    <>
      ...
      <main className="p-5">
        {items.length === 0 ? (
          ...
          <div className="rounded-2xl bg-white shadow-card border border-[#E5E5EA] overflow-hidden divide-y divide-[#E5E5EA]">
            {items.map((a, i) => {
              const Icon = ICONS[a.type] ?? Activity;
              return (
                <div key={i} className="flex items-center gap-3 p-4 hover:bg-[#FFFFFF]">
                  <div className="h-8 w-8 rounded-full bg-[#8136B2]/10 grid place-items-center flex-shrink-0">
